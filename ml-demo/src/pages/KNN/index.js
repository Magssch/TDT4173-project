...
const calulateClass = (datapoint, data, k) => {
  ...
  return [
    cl0.length > cl1.length ? 0 : cl0.length === cl1.length ? -1 : 1,
    Math.max.apply(null, nearestArr)
  ];
};
  ...
  const plotData = dataset => {
    ...
    svg
      ...
      .data(
        dataset.concat([
          point,
          { X: point.X, Y: point.Y, class: point.class, maxVal: maxVal }
        ])
      )
      .enter()
      .append("circle")
      ...
      <Grid container alignItems="center" direction="column" spacing={5}>
        <Grid item alignItems="right" xs={8}>
          <Typography align="left">
            k-Nearest Neighbors (kNN) falls within the supervised learning
            ...
            during test phase. The core of the kNN algorithm is simply measuring
            the distance between tested examples and training examples. Although
            its simplistic nature, kNN has proven to be a very effective machine
            learning algorithm
          </Typography>
          <Typography align="left">
            Here we have created an interactive model for testing how kNN works
            in practice. By moving the scroll bars on the top and right of the
            grid, it is possible to move the marker. Once the marker is placed,
            one can choose any positive value for k. Press calculate to find out
            whether the marker will be classified as either blue or red, based
            on the nearest k neighbors.
          </Typography>
        </Grid>
